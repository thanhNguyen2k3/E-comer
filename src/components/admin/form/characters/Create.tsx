...
const FormCharacter = ({ regions, visions, weapons }: Props) => {
    ...
    useEffect(() => {
        ...
        setRegion(form.getFieldValue('regionId'));
        setWeapon(form.getFieldValue('weaponId'));
        setVison(form.getFieldValue('visionId'));
    }, [form]);
    ...
    return (
        <div className="grid grid-cols-1 lg:gap-x-4 lg:grid-cols-3 gap-y-4">
            <div className="border-dashed col-span-2 relative border border-gray-500 bg-black">
                <div className="px-4 py-4 text-white">
                    ...
                    <div className="space-y-1 mt-2">
                        <div className="flex justify-between px-4">
                            ...
                            <p className="space-x-2">
                                <span className="inline-block">Region: </span>
                                <span className="text-base font-semibold">{region}</span>
                            </p>
                        </div>
                        <div className="flex justify-between px-4">
                            <p className="space-x-2">
                                <span className="inline-block">Vũ khí: </span>
                                <span className="text-base font-semibold">{weapon}</span>
                            </p>
                            <p className="space-x-2">
                                <span className="inline-block">Vision: </span>
                                <span className="text-base font-semibold">{vision}</span>
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <Form form={form} className="col-span-1" onFinish={onSubmit}>
                <InputField name={'name'} placeholder="Tên nhân vật" onChange={(e) => setName(e.target.value)} />

                <Form.Item name={'regionId'}>
                    <Select defaultValue={''} onChange={(value) => setRegion(value)}>
                ...
                <Form.Item name={'weaponId'}>
                    <Select defaultValue={''} onChange={(value) => setWeapon(value)}>
                ...
                <Form.Item name={'visionId'}>
                    <Select defaultValue={''} onChange={(value) => setVison(value)}>
